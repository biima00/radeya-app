                ...
                <div className="max-h-48 overflow-y-auto space-y-3 pr-1 scrollbar-thin">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((noti) => (
                      <div
                        key={noti.id}
                        className={`p-3 rounded-xl border flex items-start gap-2.5 transition-all text-left ${noti.type === 'sale'
      ...
      <div className="max-w-7xl mx-auto flex gap-2 overflow-x-auto py-3 px-4 no-scrollbar">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-extrabold transition-all duration-200 border ${activeTab === tab.id
    ...
    <main className="max-w-7xl mx-auto w-full px-4 py-8 flex-1 pb-28">
      ...
      {stats && activeTab === 'dashboard' && (
        <div className="space-y-8 animate-fadeIn">
          ...
          {/* Laba Bersih Card */}
          <div className={`p-8 rounded-3xl relative overflow-hidden shadow-2xl border ${stats.laba >= 0
      ...
      {activeTab === 'modal_awal' && (
        <div className="max-w-xl mx-auto bg-slate-900/30 border border-slate-850 p-6 rounded-3xl space-y-6 animate-fadeIn">
          ...
          {activeCycle.mode === 'pertanian' && (
            <div className="space-y-4 font-semibold animate-fadeIn">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 block uppercase tracking-wider">Luas Lahan (m²)</label>
                <input
                  type="number"
                  placeholder="Contoh: 500"
                  value={activeCycle.data?.modal?.luas_lahan || ''}
                  onChange={(e) => {
                    const modal = { ...(activeCycle.data?.modal || {}), luas_lahan: e.target.value };
                    handleSaveCycleData({ ...activeCycle.data, modal });
                  }}
                  className="w-full bg-slate-950/50 border border-slate-850 focus:border-teal-500 focus:outline-none rounded-xl px-4 py-3 text-slate-200 font-semibold transition-colors"
                />
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 block uppercase tracking-wider">Biaya Sewa / Persiapan Lahan (Rp)</label>
                <input
                  type="number"
                  placeholder="Contoh: 1500000"
                  value={activeCycle.data?.modal?.biaya_sewa || ''}
                  onChange={(e) => {
                    const modal = { ...(activeCycle.data?.modal || {}), biaya_sewa: e.target.value };
                    handleSaveCycleData({ ...activeCycle.data, modal });
                  }}
                  className="w-full bg-slate-950/50 border border-slate-850 focus:border-teal-500 focus:outline-none rounded-xl px-4 py-3 text-slate-200 font-semibold transition-colors"
                />
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 block uppercase tracking-wider">Biaya Pupuk & Benih Awal (Rp)</label>
                <input
                  type="number"
                  placeholder="Contoh: 500000"
                  value={activeCycle.data?.modal?.biaya_pupuk_awal || ''}
                  onChange={(e) => {
                    const modal = {
                      ...(activeCycle.data?.modal || {}),
                      biaya_pupuk_awal: e.target.value,
                      biaya_kandang: e.target.value,
                      kandang_total: e.target.value
                    };
                    handleSaveCycleData({ ...activeCycle.data, modal });
                  }}
                  className="w-full bg-slate-950/50 border border-slate-850 focus:border-teal-500 focus:outline-none rounded-xl px-4 py-3 text-slate-200 font-semibold transition-colors"
            ...
            {calendarTasks.map((task) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-4 p-5 rounded-2xl border transition-all cursor-pointer select-none ${isChecked
              ...
              {chatMessages.length === 0 ? (
                ...
                chatMessages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fadeIn`}>
                    <div className={`max-w-[85%] p-4 rounded-2xl text-xs leading-relaxed font-semibold ${msg.role === 'user'
                    ...
                    {teamMembers.map((member) => (
                      <tr key={member.id}>
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${member.role === 'OWNER'
